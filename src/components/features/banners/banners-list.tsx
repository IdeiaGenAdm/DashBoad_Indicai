...
import { deleteBanner, listBanners } from '@/services/admin-banners-fetch'
...
export function BannersList() {
  ...
  const [data, setData] = useState<BannerListItem[]>([])
  const [, setTotal] = useState(0)
  const [isLoading, setIsLoading] = useState(true)
  const [editBanner, setEditBanner] = useState<BannerListItem | null>(null)
  const [createOpen, setCreateOpen] = useState(false)
  const [confirmDelete, setConfirmDelete] = useState<BannerListItem | null>(null)
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const items = Array.isArray(res.banners)
        ? res.banners
        : ((res as { data?: BannerListItem[] }).data ?? [])
      setData(items)
        ...
        <DataTableBody>
          {data.map((item) => (
            <DataTableRow key={item.id}>
              ...
              <DataTableCell>
                {typeof item.destinatarios === 'string' ? item.destinatarios : 'todos'}
              </DataTableCell>
              <DataTableCell className="text-sm text-muted-foreground">
                {item.vigenciaInicio || item.vigenciaFim
                  ? `${item.vigenciaInicio ?? '-'} a ${item.vigenciaFim ?? '-'}`
                  : 'Sem limite'}
              </DataTableCell>
              <DataTableCell className="text-right">
                <div className="flex justify-end gap-2">
                  <Button variant="outline" size="sm" onClick={() => setEditBanner(item)}>
      ...
      <Dialog open={!!confirmDelete} onOpenChange={(open) => !open && setConfirmDelete(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {`Eliminar o banner "${confirmDelete?.titulo}"? Esta ação não pode ser revertida.`}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
